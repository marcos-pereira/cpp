...
class DQ {
 ...
 public:
  ...
  // Static
  static DQ unitDQ(const double& rot_angle, const int& x_axis,
                   const int& y_axis, const int& z_axis, const double& x_trans,
                   const double& y_trans, const double& z_trans);
  ...
  explicit DQ(const double& q0 = 0.0, const double& q1 = 0.0,
              const double& q2 = 0.0, const double& q3 = 0.0,
              const double& q4 = 0.0, const double& q5 = 0.0,
              const double& q6 = 0.0, const double& q7 = 0.0) noexcept;
  ...
  template <typename Scalar,
            typename scalar = std::enable_if<std::is_arithmetic<Scalar>::value>>
...
const DQ operator-(const DQ& dq1, const DQ& dq2) noexcept;
const DQ operator-(DQ&& rdq1, const DQ& dq2) noexcept;
// const DQ operator-(const DQ& dq1, DQ&& rdq2) noexcept; //TODO: Think of a
// smart way to implement this
const DQ operator-(DQ&& rdq1, DQ&& rdq2) noexcept;
...
template <typename Scalar, typename = typename std::enable_if<
                               std::is_arithmetic<Scalar>::value>::type>
/**
 * @brief operator + between a DQ and a Scalar.
 * @param dq a DQ.
 * @param s any Scalar (as defined by std::is_arithmetic, bools are purposedly
 * removed using static_assert.)
 * @return the sum between the DQ and the Scalar.
 */
inline const DQ operator+(const DQ& dq, const Scalar& s) noexcept {
  static_assert(!std::is_same<bool, Scalar>(),
                "Operations between DQs and bools are not supported.");
...
template <typename Scalar, typename = typename std::enable_if<
                               std::is_arithmetic<Scalar>::value>::type>
/**
 * @brief __dq_scalar_equal_impl Implementation of the operator == between a DQ
 * and a Scalar, shared between == and != operators.
 * @param dq a DQ.
 * @param s any Scalar (as defined by std::is_arithmetic, bools are purposedly
 * removed using static_assert.)
 * @return true if the DQ and Scalar are equal (up to the DQ_treshold), false
 * otherwise.
 */
inline bool __dq_scalar_equal_impl(const DQ& dq, const Scalar& s) noexcept {
  ...
  if (fabs(dq.q(0) - s) > DQ_threshold) return false;
  for (auto i = 1; i < 8; i++) {
    if (fabs(dq.q(i) - 0.0) > DQ_threshold) return false;
  }
  return true;
}
 ...
 * @param s any Scalar (as defined by std::is_arithmetic, bools are purposedly
 * removed using static_assert.)
 * @return true if the DQ and Scalar are different (up to the DQ_treshold),
 * false otherwise.
 */
inline bool operator!=(const DQ& dq, const Scalar& s) noexcept {
  return !__dq_scalar_equal_impl(dq, s);
